...
class DEBUGWIDGET_API UDebugWidgetFunctionLibrary : public UBlueprintFunctionLibrary
{
     ...
     * @param	Duration		The display duration (if Print to Screen is True). Using negative number will result in loading the duration time from the config.
     */
    UFUNCTION(BlueprintCallable, meta = (WorldContext = "WorldContextObject", CallableWithoutWorldContext, Keywords = "log print string debug widget", AdvancedDisplay = "2", DevelopmentOnly), Category = "Utilities|String")
    static void PrintString_DW(UObject* WorldContextObject, const FString& InString = FString(TEXT("Hello")), bool bPrintToScreen = true, bool bPrintToWidgets = true, FLinearColor TextColor = FLinearColor(0.0, 0.66, 1.0), float Duration = 2.f);

    /**
     * Clear all DebugWidgetComponent messages
     */
    UFUNCTION(BlueprintCallable, meta = (WorldContext = "WorldContextObject", CallableWithoutWorldContext, Keywords = "log print string debug widget", AdvancedDisplay = "2", DevelopmentOnly), Category = "Utilities|String")
    static void PrintStringClear_DW(const bool bMessagesWithKey, const bool bMessagesWithoutKey);
};
